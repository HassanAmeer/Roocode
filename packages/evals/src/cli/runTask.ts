...
export const processTask = async ({
	...
	try {
		...
		await publish({
			eventName: passed ? VibexEventName.EvalPass : VibexEventName.EvalFail,
			taskId: task.id,
		})
	} finally {
...
export const runTask = async ({ run, task, publish, logger, jobToken }: RunTaskOptions) => {
	...
	const ignoreEvents: Record<"broadcast" | "log", VibexEventName[]> = {
		broadcast: [VibexEventName.Message],
		log: [VibexEventName.TaskTokenUsageUpdated, VibexEventName.TaskAskResponded],
	}
	...
	client.on(IpcMessageType.TaskEvent, async (taskEvent) => {
		...
		if (
			eventName === VibexEventName.Message &&
		...
		if (
			...
			if (eventName === VibexEventName.Message) {
			...
			if (eventName === VibexEventName.Message && payload[0]?.message?.ask === "tool") {
				try {
			...
			} else if (eventName === VibexEventName.Message && payload[0]?.message?.ask === "command") {
				logEventName = `${eventName} (command)`
			} else if (eventName === VibexEventName.Message && payload[0]?.message?.ask === "completion_result") {
				logEventName = `${eventName} (completion_result)`
			}
			logger.info(`${logEventName} ->`, payload)
		}

		if (eventName === VibexEventName.TaskStarted) {
		...
		if (eventName === VibexEventName.TaskToolFailed) {
			const [_taskId, toolName, error] = payload
			await createToolError({ taskId: task.id, toolName, error })
		}

		if (eventName === VibexEventName.TaskTokenUsageUpdated || eventName === VibexEventName.TaskCompleted) {
		...
		if (eventName === VibexEventName.TaskAborted) {
			taskAbortedAt = Date.now()
		}

		if (eventName === VibexEventName.TaskCompleted) {
