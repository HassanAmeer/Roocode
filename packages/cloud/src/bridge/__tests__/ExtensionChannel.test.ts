...
describe("ExtensionChannel", () => {
	...
	describe("Event Listener Management", () => {
		it("should register event listeners on initialization", () => {
			...
			const expectedEvents: VibexEventName[] = [
				VibexEventName.TaskCreated,
				VibexEventName.TaskStarted,
				VibexEventName.TaskCompleted,
				VibexEventName.TaskAborted,
				VibexEventName.TaskFocused,
				VibexEventName.TaskUnfocused,
				VibexEventName.TaskActive,
				VibexEventName.TaskInteractive,
				VibexEventName.TaskResumable,
				VibexEventName.TaskIdle,
				VibexEventName.TaskPaused,
				VibexEventName.TaskUnpaused,
				VibexEventName.TaskSpawned,
				VibexEventName.TaskDelegated,
				VibexEventName.TaskDelegationCompleted,
				VibexEventName.TaskDelegationResumed,

				VibexEventName.TaskUserMessage,
				VibexEventName.TaskTokenUsageUpdated,
			]
		...
		it("should properly forward events to socket when listeners are triggered", async () => {
			...
			const taskStartedListeners = eventListeners.get(VibexEventName.TaskStarted)
		...
		it("should forward delegation events to socket", async () => {
			...
			const delegatedListeners = eventListeners.get(VibexEventName.TaskDelegated)
		...
		it("should forward TaskDelegationCompleted with correct payload", async () => {
			...
			const completedListeners = eventListeners.get(VibexEventName.TaskDelegationCompleted)
		...
		it("should forward TaskDelegationResumed with correct payload", async () => {
			...
			const resumedListeners = eventListeners.get(VibexEventName.TaskDelegationResumed)
		...
		it("should propagate all three delegation events in order", async () => {
			...
			const delegatedListener = Array.from(eventListeners.get(VibexEventName.TaskDelegated)!)[0]
			await (delegatedListener as any)("p1", "c1")

			// Trigger TaskDelegationCompleted
			const completedListener = Array.from(eventListeners.get(VibexEventName.TaskDelegationCompleted)!)[0]
			await (completedListener as any)("p1", "c1", "result")

			// Trigger TaskDelegationResumed
			const resumedListener = Array.from(eventListeners.get(VibexEventName.TaskDelegationResumed)!)[0]
