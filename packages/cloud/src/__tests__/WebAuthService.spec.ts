...
describe("WebAuthService", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(getClerkBaseUrl).mockReturnValue("https://clerk.vibex.com")
		vi.mocked(getRooCodeApiUrl).mockReturnValue("https://api.test.com")

		// Setup utils mock
		vi.mocked(getUserAgent).mockReturnValue("Vibex 1.0.0")
	...
	describe("login", () => {
		...
		it("should handle errors during login", async () => {
			...
			await expect(authService.login()).rejects.toThrow("Failed to initiate Vibex Cloud authentication")
			expect(mockLog).toHaveBeenCalledWith("[auth] Error initiating Vibex Cloud auth: Error: Crypto error")
		})
	})
	...
	describe("handleCallback", () => {
		...
		it("should handle invalid parameters", async () => {
			...
			expect(mockShowInfo).toHaveBeenCalledWith("Invalid Vibex Cloud sign in url")
		...
		it("should validate state parameter", async () => {
			...
			await expect(authService.handleCallback("code", "different-state")).rejects.toThrow(
				"Failed to handle Vibex Cloud callback",
			)
		...
		it("should successfully handle valid callback", async () => {
			...
			expect(mockShowInfo).toHaveBeenCalledWith("Successfully authenticated with Vibex Cloud")
		})
	...
	describe("logout", () => {
		...
		it("should clear credentials and call Clerk logout", async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith(
				"https://clerk.vibex.com/v1/client/sessions/test-session/remove",
				expect.objectContaining({
					method: "POST",
			...
			expect(mockShowInfo).toHaveBeenCalledWith("Logged out from Vibex Cloud")
		})
