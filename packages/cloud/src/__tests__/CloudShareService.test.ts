...
import type { SettingsService, AuthService } from "@vibex-code/types"
...
vi.mock("../Config", () => ({
	getRooCodeApiUrl: () => "https://app.vibex.com",
}))

vi.mock("../utils", () => ({
	getUserAgent: () => "Vibex 1.0.0",
}))
	...
	describe("shareTask", () => {
		it("should share task with organization visibility and copy to clipboard", async () => {
			const mockResponseData = {
				success: true,
				shareUrl: "https://app.vibex.com/share/abc123",
			}
			...
			expect(result.shareUrl).toBe("https://app.vibex.com/share/abc123")

			expect(mockFetch).toHaveBeenCalledWith("https://app.vibex.com/api/extension/share", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: "Bearer session-token",
					"User-Agent": "Vibex 1.0.0",
				},
				body: JSON.stringify({
					taskId: "task-123",
			...
			expect(vscode.env.clipboard.writeText).toHaveBeenCalledWith("https://app.vibex.com/share/abc123")
		})
