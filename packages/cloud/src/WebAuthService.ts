...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			this.log(`[auth] Error initiating Vibex Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate Vibex Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from Vibex Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with Vibex Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid Vibex Cloud sign in url")
			}
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with Vibex Cloud")
			}

			this.log("[auth] Successfully authenticated with Vibex Cloud")
		} catch (error) {
			this.log(`[auth] Error handling Vibex Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle Vibex Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from Vibex Cloud")
			}

			this.log("[auth] Logged out from Vibex Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from Vibex Cloud: ${error}`)
			throw new Error(`Failed to log out from Vibex Cloud: ${error}`)
		}
	}
