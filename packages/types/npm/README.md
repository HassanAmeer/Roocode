...
```typescript
import { VibexAPI } from "@roo-code/types"

const extension = vscode.extensions.getExtension<VibexAPI>("Vibex.roo-cline")
...
**NOTE:** To ensure that the `Vibex.roo-cline` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

```json
"extensionDependencies": ["Vibex.roo-cline"]
```
