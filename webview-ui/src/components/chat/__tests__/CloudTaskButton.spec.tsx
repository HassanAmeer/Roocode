...
vi.mock("@vibex-code/cloud/src/config", () => ({
	getRooCodeApiUrl: vi.fn(() => "https://app.vibex.com"),
}))
...
describe("CloudTaskButton", () => {
	...
	beforeEach(() => {
		...
		mockUseExtensionState.mockReturnValue({
			...
			cloudApiUrl: "https://app.vibex.com",
		} as any)
	})
	...
	test("displays correct cloud URL in dialog", async () => {
		...
		await waitFor(() => {
			const input = screen.getByDisplayValue("https://app.vibex.com/task/test-task-id")
	...
	test("uses correct URL from getRooCodeApiUrl", async () => {
		...
		vi.doMock("@vibex-code/cloud/src/config", () => ({
			getRooCodeApiUrl: vi.fn(() => "https://custom.vibex.com"),
		}))
