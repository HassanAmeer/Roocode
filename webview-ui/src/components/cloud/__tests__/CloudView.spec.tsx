	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"cloud:signIn": "Connect to Vibex Cloud",
				"cloud:cloudBenefitsTitle": "Try Vibex Cloud",
				"cloud:cloudBenefitProvider": "Access free and paid models that work great with Vibex",
				...
				"cloud:visitCloudWebsite": "Visit Vibex Cloud",
				"cloud:taskSync": "Task sync",
				"cloud:taskSyncDescription": "Sync your tasks for viewing and sharing on Vibex Cloud",
				"cloud:taskSyncManagedByOrganization": "Task sync is managed by your organization",
				"cloud:remoteControl": "VibexRemote Control",
				"cloud:remoteControlDescription":
					"Enable following and interacting with tasks in this workspace with Vibex Cloud",
				"cloud:remoteControlRequiresTaskSync": "Task sync must be enabled to use VibexRemote Control",
				"cloud:usageMetricsAlwaysReported": "Model usage info is always reported when logged in",
				"cloud:profilePicture": "Profile picture",
				"cloud:cloudUrlPillLabel": "Vibex Cloud URL: ",
			}
...
const mockExtensionState = {
	...
	featureVibexRemoteControlEnabled: true, // Default to true for tests
	setFeatureVibexRemoteControlEnabled: vi.fn(),
}
...
describe("CloudView", () => {
	it("should display benefits when user is not authenticated", () => {
		render(<CloudView userInfo={null} isAuthenticated={false} cloudApiUrl="https://app.vibex.com" />)

		// Check that the benefits section is displayed
		expect(screen.getByRole("heading", { name: "Try Vibex Cloud" })).toBeInTheDocument()
		expect(screen.getByText("Access free and paid models that work great with Vibex")).toBeInTheDocument()
	...
	it("should not display benefits when user is authenticated", () => {
		...
		expect(screen.queryByText("Access free and paid models that work great with Vibex")).not.toBeInTheDocument()
	...
	it("should display remote control toggle when user has extension bridge enabled and vibexmote control enabled", () => {
		...
		expect(screen.getByText("VibexRemote Control")).toBeInTheDocument()
		expect(
			screen.getByText("Enable following and interacting with tasks in this workspace with Vibex Cloud"),
		).toBeInTheDocument()
	})
	...
	it("should not display remote control toggle when vibexmote control is disabled", () => {
		...
		mockExtensionState.featureVibexRemoteControlEnabled = originalFeatureVibexRemoteControlEnabled
	})
	...
	it("should display remote control toggle for organization users (simulating backend logic)", () => {
		...
		// Organization users are treated as having featureVibexRemoteControlEnabled true
		const originalFeatureVibexRemoteControlEnabled = mockExtensionState.featureVibexRemoteControlEnabled
		mockExtensionState.featureVibexRemoteControlEnabled = true // Simulating ClineProvider logic for org users
		...
		// (The ClineProvider would set featureVibexRemoteControlEnabled to true for org users)
		expect(screen.getByTestId("remote-control-toggle")).toBeInTheDocument()
		expect(screen.getByText("VibexRemote Control")).toBeInTheDocument()
	...
	it("should not display cloud URL pill when pointing to production", () => {
		...
		expect(screen.queryByText(/Vibex Cloud URL:/)).not.toBeInTheDocument()
	})
	...
	it("should display cloud URL pill when pointing to non-production environment", () => {
		...
		render(<CloudView userInfo={mockUserInfo} isAuthenticated={true} cloudApiUrl="https://staging.vibex.com" />)

		// Check that the cloud URL pill is displayed with the staging URL
		expect(screen.getByText(/Vibex Cloud URL:/)).toBeInTheDocument()
		expect(screen.getByText("https://staging.vibex.com")).toBeInTheDocument()
	})

	it("should display cloud URL pill for non-authenticated users when not pointing to production", () => {
		render(<CloudView userInfo={null} isAuthenticated={false} cloudApiUrl="https://dev.vibex.com" />)

		// Check that the cloud URL pill is displayed even when not authenticated
		expect(screen.getByText(/Vibex Cloud URL:/)).toBeInTheDocument()
		expect(screen.getByText("https://dev.vibex.com")).toBeInTheDocument()
	})
