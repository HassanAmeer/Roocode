...
vi.mock("@/utils/docLinks", () => ({
	buildDocLink: (path: string, source: string) => `https://docs.vibex.com/${path}?utm_source=${source}`,
}))
...
describe("WelcomeViewProvider", () => {
	...
	describe("Provider Selection Screen", () => {
		...
		it("shows radio buttons for Vibex and Custom providers", () => {
			renderWelcomeViewProvider()
		...
		it("Vibex provider is selected by default", () => {
			renderWelcomeViewProvider()
			navigateToProviderSelection()
		...
		it("triggers auth when Get Started is clicked on Vibex provider (not authenticated)", () => {
		...
		it("saves config immediately when Get Started is clicked on Vibex provider (already authenticated)", () => {
	...
	describe("Auth In Progress State", () => {
		...
		it("returns to provider selection when Go Back is clicked (auth from provider selection)", () => {
			...
			// Start auth from provider selection (Vibex is selected by default)
			const getStartedButton = screen.getByTestId("button-primary")
			fireEvent.click(getStartedButton)
