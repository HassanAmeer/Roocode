...
describe("History resume delegation - parent metadata transitions", () => {
	...
	it("reopenParentFromDelegation emits events in correct order: TaskDelegationCompleted → TaskDelegationResumed", async () => {
		...
		expect(eventNames).toContain(VibexEventName.TaskDelegationCompleted)
		expect(eventNames).toContain(VibexEventName.TaskDelegationResumed)

		// CRITICAL: verify ordering (TaskDelegationCompleted before TaskDelegationResumed)
		const completedIdx = emitSpy.mock.calls.findIndex((c) => c[0] === VibexEventName.TaskDelegationCompleted)
		const resumedIdx = emitSpy.mock.calls.findIndex((c) => c[0] === VibexEventName.TaskDelegationResumed)
	...
	it("reopenParentFromDelegation does NOT emit TaskPaused or TaskUnpaused (new flow only)", async () => {
		...
		const eventNames = emitSpy.mock.calls.map((c) => c[0])
		expect(eventNames).not.toContain(VibexEventName.TaskPaused)
		expect(eventNames).not.toContain(VibexEventName.TaskUnpaused)
		expect(eventNames).not.toContain(VibexEventName.TaskSpawned)
	})
