...
import type { AuthState } from "@vibex-code/types"
...
vi.mock("../core/webview/ClineProvider", async () => {
	const { BridgeOrchestrator } = await import("@vibex-code/cloud")
	const mockInstance = {
...
describe("extension.ts", () => {
	...
	test("authStateChangedHandler calls BridgeOrchestrator.disconnect when logged-out event fires", async () => {
		const { CloudService, BridgeOrchestrator } = await import("@vibex-code/cloud")
	...
	describe("Vibex model cache refresh on auth state change (ROO-202)", () => {
		beforeEach(() => {
			vi.resetModules()
		...
		test("flushModels is called when auth state changes to logged-out", async () => {
			...
			expect(flushModels).toHaveBeenCalledWith({ provider: "vibex" }, false)
		})
	})
})
