...
import { VibeXTerminal, VibeXTerminalProvider } from "./types"
...
export class TerminalRegistry {
	private static terminals: VibeXTerminal[] = []
		...
		try {
			const startDisposable = vscode.window.onDidStartTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionStartEvent) => {
					...
					if (terminal) {
						...
						console.error(
							"[onDidStartTerminalShellExecution] Shell execution started, but not from a Vibex-registered terminal:",
							e,
						)
					}
				},
			...
			const endDisposable = vscode.window.onDidEndTerminalShellExecution?.(
				async (e: vscode.TerminalShellExecutionEndEvent) => {
					...
					if (!terminal) {
						console.error(
							"[onDidEndTerminalShellExecution] Shell execution ended, but not from a Vibex-registered terminal:",
							e,
						)
	...
	public static createTerminal(cwd: string, provider: VibeXTerminalProvider): VibeXTerminal {
		let newTerminal
	...
	public static async getOrCreateTerminal(
		cwd: string,
		taskId?: string,
		provider: VibeXTerminalProvider = "vscode",
	): Promise<VibeXTerminal> {
		const terminals = this.getAllTerminals()
		let terminal: VibeXTerminal | undefined
	 ...
	 */
	public static getTerminals(busy: boolean, taskId?: string): VibeXTerminal[] {
	 ...
	 * @returns Array of Terminal objects
	 */
	public static getBackgroundTerminals(busy?: boolean): VibeXTerminal[] {
		return this.getAllTerminals().filter((t) => {
	...
	private static getAllTerminals(): VibeXTerminal[] {
		this.terminals = this.terminals.filter((t) => !t.isClosed())
		return this.terminals
	}

	private static getTerminalById(id: number): VibeXTerminal | undefined {
	 ...
	 */
	private static getTerminalByVSCETerminal(vsceTerminal: vscode.Terminal): VibeXTerminal | undefined {
