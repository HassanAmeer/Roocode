...
import type { OrganizationSettings, MarketplaceItem, MarketplaceItemType, McpMarketplaceItem } from "@vibex-code/types"
...
export class MarketplaceManager {
	...
	private async checkProjectInstallations(metadata: Record<string, { type: string }>): Promise<void> {
		try {
			...
			// Check modes in .vibexmodes
			const projectModesPath = path.join(workspaceFolder.uri.fsPath, ".vibexmodes")
			try {
			...
			// Check MCPs in .vibex/mcp.json
			const projectMcpPath = path.join(workspaceFolder.uri.fsPath, ".vibex", "mcp.json")
			try {
