...
import { VibexIgnoreController } from "../../core/ignore/VibexIgnoreController"
...
export async function parseSourceCodeDefinitionsForFile(
	...
	if (ext === ".md" || ext === ".markdown") {
		...
		if (vibexIgnoreController && !vibexIgnoreController.validateAccess(filePath)) {
			return undefined
		}
	...
	const definitions = await parseFile(filePath, languageParsers, vibexIgnoreController)
	if (definitions) {
		return `# ${path.basename(filePath)}\n${definitions}`
	}
...
2. Create a query using a language-specific query string, and run it against the AST's vibext node to capture specific syntax elements.
 ...
 * @param vibexIgnoreController - Optional controller to check file access permissions
