...
export class MdmService {
	...
	private getMdmConfigPath(): string {
		...
		switch (platform) {
			case "win32": {
				// Windows: %ProgramData%\Vibex\mdm.json or mdm.dev.json
				const programData = process.env.PROGRAMDATA || "C:\\ProgramData"
				return path.join(programData, "Vibex", configFileName)
			}

			case "darwin":
				// macOS: /Library/Application Support/Vibex/mdm.json or mdm.dev.json
				return `/Library/Application Support/Vibex/${configFileName}`

			case "linux":
			default:
				// Linux: /etc/vibex/mdm.json or mdm.dev.json
				return `/etc/vibex/${configFileName}`
		}
	}
