...
vi.mock("@vibex-code/cloud", () => ({
	...
	PRODUCTION_CLERK_BASE_URL: "https://clerk.vibex.com",
}))
	...
	t: vi.fn((key: string) => {
		const translations: Record<string, string> = {
			"mdm.errors.cloud_auth_required":
				"Your organization requires Vibex Cloud authentication. Please sign in to continue.",
			"mdm.errors.organization_mismatch":
				"You must be authenticated with your organization's Vibex Cloud account.",
...
import { CloudService, getClerkBaseUrl, PRODUCTION_CLERK_BASE_URL } from "@vibex-code/cloud"
...
describe("MdmService", () => {
	...
	beforeEach(() => {
		...
		mockGetClerkBaseUrl.mockReturnValue("https://dev.clerk.vibex.com")
	...
	describe("platform-specific config paths", () => {
		...
		it("should use correct path for Windows in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "Vibex", "mdm.json"))
		})
		...
		it("should use correct path for Windows in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "Vibex", "mdm.dev.json"))
		})
		...
		it("should use correct path for macOS in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/Vibex/mdm.json")
		})
		...
		it("should use correct path for macOS in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/Vibex/mdm.dev.json")
		})
		...
		it("should use correct path for Linux in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/vibex/mdm.json")
		})
		...
		it("should use correct path for Linux in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/vibex/mdm.dev.json")
		})
	...
	describe("compliance checking", () => {
		...
		it("should be non-compliant when not authenticated", async () => {
			...
			if (!compliance.compliant) {
				expect(compliance.reason).toContain("Your organization requires Vibex Cloud authentication")
			}
		})
