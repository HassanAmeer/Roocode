...
/**
 * Fetches available models from the Vibex Cloud provider
 *
 * @param baseUrl The base URL of the Vibex Cloud provider
 * @param apiKey The API key (session token) for the Vibex Cloud provider
	...
	try {
		...
		try {
			...
			if (!parsed.success) {
				...
				throw new Error("Failed to fetch Vibex Cloud models: Unexpected response format.")
			}
		...
		console.error("[getRooModels] Error fetching Vibex Cloud models:", {
		...
		if (error.name === "AbortError") {
			throw new Error("Failed to fetch Vibex Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (error.message?.includes("HTTP")) {
			throw new Error(`Failed to fetch Vibex Cloud models: ${error.message}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch Vibex Cloud models: No response from server. Check Vibex Cloud server status and base URL.",
			)
		}

		throw new Error(`Failed to fetch Vibex Cloud models: ${error.message || "An unknown error occurred."}`)
	}
}
