...
export class VibeXHandler extends BaseOpenAiCompatibleProvider<string> {
	...
	constructor(options: ApiHandlerOptions) {
		...
		let baseURL = process.env.ROO_CODE_PROVIDER_URL ?? "https://api.vibex.com/proxy"
	...
	override async *createMessage(
		...
		try {
			...
			const headers: Record<string, string> = {
				"X-Vibex-App-Version": Package.version,
			}

			if (metadata?.taskId) {
				headers["X-Vibex-Task-ID"] = metadata.taskId
			}
	...
	/**
	 * Generate an image using Vibex Cloud's image generation API
