...
export class AwsBedrockHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ProviderSettings) {
		...
		const clientConfig: BedrockRuntimeClientConfig = {
			userAgentAppId: `Vibex#${Package.version}`,
	...
	private parseArn(arn: string, region?: string) {
		/*
		 * VIA Vibex analysis: platform-independent Regex. It's designed to parse Amazon Bedrock ARNs and doesn't rely on any platform-specific features
	...
	getModelById(modelId: string, modelType?: string): { id: BedrockModelId | string; info: ModelInfo } {
		...
		if (this.options.modelContextWindow && this.options.modelContextWindow > 0) {
			model.info.contextWindow = this.options.modelContextWindow
		}
