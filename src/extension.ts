...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize the provider *before* the Vibex Cloud service.
	const provider = new ClineProvider(context, outputChannel, "sidebar", contextProxy, mdmService)

	// Initialize Vibex Cloud service.
	...
	authStateChangedHandler = async (data: { state: AuthState; previousState: AuthState }) => {
		...
		// Handle Vibex models cache based on auth state (ROO-202)
		const handleRooModelsCache = async () => {
			try {
				...
				cloudLogger(
					`[authStateChangedHandler] Failed to handle Vibex models cache: ${error instanceof Error ? error.message : String(error)}`,
				)
			}
		}
	...
	// Allows other extensions to activate once Vibex is ready.
	vscode.commands.executeCommand(`${Package.name}.activationCompleted`)

	// Implements the `VibexAPI` interface.
