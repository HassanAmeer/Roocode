...
import { isLanguage } from "@vibex-code/types"

import type { SystemPromptSettings } from "../types"
import { getEffectiveProtocol, isNativeProtocol } from "@vibex-code/types"

import { LANGUAGES } from "../../../shared/language"
import { getVibexDirectoriesForCwd, getGlobalVibexDirectory } from "../../../services/vibex-config"
...
export async function loadRuleFiles(cwd: string): Promise<string> {
	const rules: string[] = []
	const vibexDirectories = getVibexDirectoriesForCwd(cwd)

	// Check for .vibex/rules/ directories in order (global first, then project-local)
	for (const vibexDir of vibexDirectories) {
		const rulesDir = path.join(vibexDir, "rules")
	...
	// If we found rules in .vibex/rules/ directories, return them
	if (rules.length > 0) {
		return "\n" + rules.join("\n\n")
	}

	// Fall back to existing behavior for legacy .vibexrules/.clinerules files
	const ruleFiles = [".vibexrules", ".clinerules"]
...
/**
 * Load AGENTS.md or AGENT.md file from the project vibext if it exists
...
export async function addCustomInstructions(
	...
	options: {
		language?: string
		vibexIgnoreInstructions?: string
	...
	if (mode) {
		...
		// Check for .vibex/rules-${mode}/ directories in order (global first, then project-local)
		for (const vibexDir of vibexDirectories) {
			const modeRulesDir = path.join(vibexDir, `rules-${mode}`)
		...
		// If we found mode-specific rules in .vibex/rules-${mode}/ directories, use them
		if (modeRules.length > 0) {
			modeRuleContent = "\n" + modeRules.join("\n\n")
			usedRuleFile = `rules-${mode} directories`
		} else {
			// Fall back to existing behavior for legacy files
			const vibexModeRuleFile = `.vibexrules-${mode}`
			modeRuleContent = await safeReadFile(path.join(cwd, vibexModeRuleFile))
			if (modeRuleContent) {
				usedRuleFile = vibexModeRuleFile
			} else {
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".vibex", `rules-${mode}`))) {
	...
	if (options.vibexIgnoreInstructions) {
		rules.push(options.vibexIgnoreInstructions)
	}
