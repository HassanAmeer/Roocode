...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	return path.join(cwd, ".vibex", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .vibex/system-prompt-[mode slug]
...
export async function ensureRooDirectory(cwd: string): Promise<void> {
	const vibexDir = path.join(cwd, ".vibex")

	// Check if directory already exists
	if (await fileExistsAtPath(vibexDir)) {
		return
	}
