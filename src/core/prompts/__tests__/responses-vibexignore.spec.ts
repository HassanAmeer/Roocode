...
describe("RooIgnore Response Formatting", () => {
	...
	describe("formatResponse.vibexIgnoreError", () => {
		/**
		...
		it("should include the file path in the error message", () => {
			...
			for (const testPath of paths) {
				const errorMessage = formatResponse.vibexIgnoreError(testPath)
	...
	describe("formatResponse.formatFilesList with VibexIgnoreController", () => {
		...
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller
			const controller = new VibexIgnoreController(TEST_CWD)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with VibexIgnoreController
		 */
		it("should handle truncation with VibexIgnoreController", async () => {
			// Create controller
			const controller = new VibexIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with VibexIgnoreController", async () => {
			// Create controller
			const controller = new VibexIgnoreController(TEST_CWD)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .vibexignore instructions for the LLM", async () => {
			// Create controller
			const controller = new VibexIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .vibexignore exists", async () => {
			// Set up no .vibexignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .vibexignore
			const controller = new VibexIgnoreController(TEST_CWD)
			await controller.initialize()
