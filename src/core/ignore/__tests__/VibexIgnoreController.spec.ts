// npx vitest core/ignore/__tests__/VibexIgnoreController.spec.ts

import type { Mock } from "vitest"

import { VibexIgnoreController, LOCK_TEXT_SYMBOL } from "../VibexIgnoreController"
...
describe("VibexIgnoreController", () => {
	const TEST_CWD = "/test/path"
	let controller: VibexIgnoreController
	...
	beforeEach(() => {
		...
		// Create controller
		controller = new VibexIgnoreController(TEST_CWD)
	})

	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .vibexignore exists
		 */
		it("should load .vibexignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .vibexignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".vibexignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".vibexignore"), "utf8")

			// Verify content was stored
			expect(controller.vibexIgnoreContent).toBe("node_modules\n.git\nsecrets.json")
		...
		/**
		 * Tests the controller behavior when .vibexignore doesn't exist
		 */
		it("should allow all access when .vibexignore doesn't exist", async () => {
			// Setup mocks to simulate missing .vibexignore file
			...
			expect(controller.vibexIgnoreContent).toBeUndefined()
		 ...
		 */
		it("should set up file watcher for .vibexignore changes", async () => {
		 ...
		 */
		it("should handle errors when loading .vibexignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .vibexignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .vibexignore exists
		 */
		it("should allow all access when no .vibexignore content", async () => {
			// Create a new controller with no .vibexignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new VibexIgnoreController(TEST_CWD)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .vibexignore exists
		 */
		it("should allow all commands when no .vibexignore exists", async () => {
			// Create a new controller with no .vibexignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new VibexIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .vibexignore
		 */
		it("should generate formatted instructions when .vibexignore exists", async () => {
			// Setup .vibexignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .vibexignore exists
		 */
		it("should return undefined when no .vibexignore exists", async () => {
			// Setup no .vibexignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .vibexignore is created
		 */
		it("should reload .vibexignore when file is created", async () => {
			// Setup initial state without .vibexignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .vibexignore
			controller = new VibexIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .vibexignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .vibexignore is changed
		 */
		it("should reload .vibexignore when file is changed", async () => {
			// Setup initial state with .vibexignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .vibexignore is deleted
		 */
		it("should reset when .vibexignore is deleted", async () => {
			// Setup initial state with .vibexignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
