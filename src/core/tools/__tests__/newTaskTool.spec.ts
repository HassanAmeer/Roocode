...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("vibex-cline")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (vibex-code-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "vibex-code-nightly"
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("vibex-code-nightly")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})
	})
