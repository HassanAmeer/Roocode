...
vi.mock("../../prompts/responses", () => ({
	formatResponse: {
		...
		vibexIgnoreError: vi.fn(
			(path: string) =>
				`Access to ${path} is blocked by the .vibexignore file settings. You must try to continue in the task without using this file, or ask the user to update the .vibexignore file.`,
		),
...
describe("read_file tool output structure", () => {
	...
	async function executeReadFileTool(
		...
		mockCline.vibexIgnoreController.validateAccess = vi.fn().mockReturnValue(validateAccess)
	...
	describe("Error Handling Tests", () => {
		...
		it("should include error for VibeXIgnore error", async () => {
			...
			// Verify - native format for error
			expect(result).toBe(
				`File: ${testFilePath}\nError: Access to ${testFilePath} is blocked by the .vibexignore file settings. You must try to continue in the task without using this file, or ask the user to update the .vibexignore file.`,
			)
		})
	})
})
