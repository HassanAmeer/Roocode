...
async function getFileOrFolderContent(
	...
	try {
		...
		if (stats.isFile()) {
			if (vibexIgnoreController && !vibexIgnoreController.validateAccess(absPath)) {
				return `(File ${mentionPath} is ignored by .vibexignore)`
			}
			try {
			...
			for (let index = 0; index < entries.length; index++) {
				...
				if (vibexIgnoreController) {
					isIgnored = !vibexIgnoreController.validateAccess(entryPath)
				}
