	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	constructor(
		...
		this.taskCreationCallback = (instance: Task) => {
			this.emit(VibexEventName.TaskCreated, instance)

			// Create named listener functions so we can remove them later.
			const onTaskStarted = () => this.emit(VibexEventName.TaskStarted, instance.taskId)
			const onTaskCompleted = (taskId: string, tokenUsage: TokenUsage, toolUsage: ToolUsage) =>
				this.emit(VibexEventName.TaskCompleted, taskId, tokenUsage, toolUsage)
			const onTaskAborted = async () => {
				this.emit(VibexEventName.TaskAborted, instance.taskId)
			...
			const onTaskFocused = () => this.emit(VibexEventName.TaskFocused, instance.taskId)
			const onTaskUnfocused = () => this.emit(VibexEventName.TaskUnfocused, instance.taskId)
			const onTaskActive = (taskId: string) => this.emit(VibexEventName.TaskActive, taskId)
			const onTaskInteractive = (taskId: string) => this.emit(VibexEventName.TaskInteractive, taskId)
			const onTaskResumable = (taskId: string) => this.emit(VibexEventName.TaskResumable, taskId)
			const onTaskIdle = (taskId: string) => this.emit(VibexEventName.TaskIdle, taskId)
			const onTaskPaused = (taskId: string) => this.emit(VibexEventName.TaskPaused, taskId)
			const onTaskUnpaused = (taskId: string) => this.emit(VibexEventName.TaskUnpaused, taskId)
			const onTaskSpawned = (taskId: string) => this.emit(VibexEventName.TaskSpawned, taskId)
			const onTaskUserMessage = (taskId: string) => this.emit(VibexEventName.TaskUserMessage, taskId)
			const onTaskTokenUsageUpdated = (taskId: string, tokenUsage: TokenUsage, toolUsage: ToolUsage) =>
				this.emit(VibexEventName.TaskTokenUsageUpdated, taskId, tokenUsage, toolUsage)

			// Attach the listeners.
			instance.on(VibexEventName.TaskStarted, onTaskStarted)
			instance.on(VibexEventName.TaskCompleted, onTaskCompleted)
			instance.on(VibexEventName.TaskAborted, onTaskAborted)
			instance.on(VibexEventName.TaskFocused, onTaskFocused)
			instance.on(VibexEventName.TaskUnfocused, onTaskUnfocused)
			instance.on(VibexEventName.TaskActive, onTaskActive)
			instance.on(VibexEventName.TaskInteractive, onTaskInteractive)
			instance.on(VibexEventName.TaskResumable, onTaskResumable)
			instance.on(VibexEventName.TaskIdle, onTaskIdle)
			instance.on(VibexEventName.TaskPaused, onTaskPaused)
			instance.on(VibexEventName.TaskUnpaused, onTaskUnpaused)
			instance.on(VibexEventName.TaskSpawned, onTaskSpawned)
			instance.on(VibexEventName.TaskUserMessage, onTaskUserMessage)
			instance.on(VibexEventName.TaskTokenUsageUpdated, onTaskTokenUsageUpdated)

			// Store the cleanup functions for later removal.
			this.taskEventListeners.set(instance, [
				() => instance.off(VibexEventName.TaskStarted, onTaskStarted),
				() => instance.off(VibexEventName.TaskCompleted, onTaskCompleted),
				() => instance.off(VibexEventName.TaskAborted, onTaskAborted),
				() => instance.off(VibexEventName.TaskFocused, onTaskFocused),
				() => instance.off(VibexEventName.TaskUnfocused, onTaskUnfocused),
				() => instance.off(VibexEventName.TaskActive, onTaskActive),
				() => instance.off(VibexEventName.TaskInteractive, onTaskInteractive),
				() => instance.off(VibexEventName.TaskResumable, onTaskResumable),
				() => instance.off(VibexEventName.TaskIdle, onTaskIdle),
				() => instance.off(VibexEventName.TaskUserMessage, onTaskUserMessage),
				() => instance.off(VibexEventName.TaskPaused, onTaskPaused),
				() => instance.off(VibexEventName.TaskUnpaused, onTaskUnpaused),
				() => instance.off(VibexEventName.TaskSpawned, onTaskSpawned),
				() => instance.off(VibexEventName.TaskTokenUsageUpdated, onTaskTokenUsageUpdated),
			])
		}

		// Initialize Vibex Cloud profile sync.
	...
	async addClineToStack(task: Task) {
		...
		task.emit(VibexEventName.TaskFocused)
	...
	async removeClineFromStack() {
		...
		if (task) {
			task.emit(VibexEventName.TaskUnfocused)
	...
	*/
		...
		*/
        ...
        <html lang="en">
          <head>
            ...
            <meta http-equiv="Content-Security-Policy" content="default-src 'none'; font-src ${webview.cspSource} data:; style-src ${webview.cspSource} 'unsafe-inline'; img-src ${webview.cspSource} https://storage.googleapis.com https://img.clerk.com data:; media-src ${webview.cspSource}; script-src ${webview.cspSource} 'wasm-unsafe-eval' 'nonce-${nonce}' https://ph.vibex.com 'strict-dynamic'; connect-src ${webview.cspSource} ${openRouterDomain} https://api.requesty.ai https://ph.vibex.com;">
            <link rel="stylesheet" type="text/css" href="${stylesUri}">
			<link href="${codiconsUri}" rel="stylesheet" />
			<script nonce="${nonce}">
				window.IMAGES_BASE_URI = "${imagesUri}"
	...
	public async handleModeSwitch(newMode: Mode) {
		...
		if (task) {
			...
			task.emit(VibexEventName.TaskModeSwitched, task.taskId, newMode)
		...
		this.emit(VibexEventName.ModeChanged, newMode)
	...
	async activateProviderProfile(args: { name: string } | { id: string }) {
		...
		if (providerSettings.apiProvider) {
			this.emit(VibexEventName.ProviderProfileChanged, { name, provider: providerSettings.apiProvider })
		}
	}
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		if (process.platform === "win32") {
			// Windows: %APPDATA%\Vibex\MCP
			mcpServersDir = path.join(os.homedir(), "AppData", "Roaming", "Vibex", "MCP")
		} else if (process.platform === "darwin") {
			// macOS: ~/Documents/Cline/MCP
			mcpServersDir = path.join(os.homedir(), "Documents", "Cline", "MCP")
		} else {
			// Linux: ~/.local/share/Cline/MCP
			mcpServersDir = path.join(os.homedir(), ".local", "share", "Vibex", "MCP")
		}
		...
		try {
			...
			return path.join(os.homedir(), ".vibex", "mcp")
		}
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		const {
			...
			featureVibexRemoteControlEnabled,
	...
	public async setValue<K extends keyof VibexSettings>(key: K, value: VibexSettings[K]) {
		await this.contextProxy.setValue(key, value)
	}

	public getValue<K extends keyof VibexSettings>(key: K) {
		return this.contextProxy.getValue(key)
	}
	...
	public async setValues(values: VibexSettings) {
	...
	public async delegateParentAndOpenChild(params: {
		...
		try {
			this.emit(VibexEventName.TaskDelegated, parentTaskId, child.taskId)
		} catch {
			// non-fatal
		}
	...
	public async reopenParentFromDelegation(params: {
		...
		try {
			this.emit(VibexEventName.TaskDelegationCompleted, parentTaskId, childTaskId, completionResultSummary)
		} catch {
			// non-fatal
		}
		...
		try {
			this.emit(VibexEventName.TaskDelegationResumed, parentTaskId, childTaskId)
		} catch {
			// non-fatal
		}
	}
