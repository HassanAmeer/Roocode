...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "updateSettings":
			if (message.updatedSettings) {
				for (const [key, value] of Object.entries(message.updatedSettings)) {
					...
					await provider.contextProxy.setValue(key as keyof VibexSettings, newValue)
				}
		...
		case "requestRooCreditBalance": {
			// Fetch Vibex credit balance using CloudAPI
		...
		case "openKeyboardShortcuts": {
			// Open VSCode keyboard shortcuts settings and optionally filter to show the Vibex commands
