...
describe("generateErrorDiagnostics", () => {
	...
	it("generates a diagnostics file with error metadata and history", async () => {
		...
		expect(result.filePath).toContain("vibex-diagnostics-")
		...
		expect(String(writtenPath)).toContain("vibex-diagnostics-test-tas")
		expect(String(writtenContent)).toContain(
			"// Please share this file with Vibex Support (support@vibex.com) to diagnose the issue faster",
		)
