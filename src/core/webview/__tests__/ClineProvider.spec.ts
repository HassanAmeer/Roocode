// pnpm --filter vibex-cline test core/webview/__tests__/ClineProvider.spec.ts
...
vi.mock("@vibex-code/cloud", () => ({
	...
	getRooCodeApiUrl: vi.fn().mockReturnValue("https://app.vibex.com"),
}))
...
describe("ClineProvider", () => {
	...
	test("resolveWebviewView sets up webview correctly in development mode even if local server is not running", async () => {
		...
		expect(mockWebviewView.webview.html).toContain(
			"connect-src vscode-webview://test-csp-source https://openrouter.ai https://api.requesty.ai https://ph.vibex.com",
		)
	...
	test("postMessageToWebview sends message to webview", async () => {
		...
		const mockState: ExtensionState = {
			...
			featureVibexRemoteControlEnabled: false,
	...
	describe("clearTask message handler", () => {
		...
		test("calls clearTask even with parent task (delegation via metadata)", async () => {
			...
			;(childTask as any).vibextTask = parentTask
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(mockedFs.mkdir).toHaveBeenCalledWith("/test/workspace/.vibex", { recursive: true })

		// Verify file was created with default content
		expect(safeWriteJson).toHaveBeenCalledWith("/test/workspace/.vibex/mcp.json", { mcpServers: {} })

		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.vibex/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			expect.stringContaining("Failed to create or open .vibex/mcp.json"),
		)
	})
})
...
describe("getTelemetryProperties", () => {
	...
	describe("cloud authentication telemetry", () => {
		...
		test("includes cloud authentication property when user is authenticated", async () => {
			...
			const { CloudService } = await import("@vibex-code/cloud")
...
describe("ClineProvider - Router Models", () => {
	...
	test("handles requestRouterModels with individual provider failures", async () => {
		...
		vi.mocked(getModels)
			...
			.mockResolvedValueOnce(mockModels) // vibex success
