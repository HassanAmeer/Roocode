...
describe("CustomModesManager", () => {
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .vibexmodes file and JSON for global customModes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]

			const vibexmodesModes = [{ slug: "mode2", name: "Mode 2", roleDefinition: "Role 2", groups: ["read"] }]
			...
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				...
				if (path === mockRoomodes) {
					return yaml.stringify({ customModes: vibexmodesModes })
				}
				throw new Error("File not found")
			})
		...
		it("should merge modes with .vibexmodes taking precedence", async () => {
			...
			// Should contain 3 modes (mode1 from settings, mode2 and mode3 from vibexmodes)
			expect(modes).toHaveLength(3)
			expect(modes.map((m) => m.slug)).toEqual(["mode2", "mode3", "mode1"])

			// mode2 should come from .vibexmodes since it takes precedence
		...
		it("should handle missing .vibexmodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .vibexmodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .vibexmodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .vibexmodes precedence", async () => {
			...
			let vibexmodesContent = { customModes: vibexmodesModes }
			...
			;(fs.writeFile as Mock).mockImplementation(async (path: string, content: string, _encoding?: string) => {
				...
				if (path === mockRoomodes) {
					vibexmodesContent = yaml.parse(content)
				}
			...
			// Should update global state with merged modes where .vibexmodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .vibexmodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .vibexmodes file when adding project-specific mode", async () => {
			...
			// Mock .vibexmodes to not exist initially
			let vibexmodesContent: any = null
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
			...
			// Verify .vibexmodes was created with the project mode
			...
			// Verify the content written to .vibexmodes
			expect(vibexmodesContent).toEqual({
				customModes: [
					expect.objectContaining({
						slug: "project-mode",
						name: "Project Mode",
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				;(fs.readFile as Mock).mockImplementation(async (path: string) => {
					...
					if (path === mockRoomodes && vibexmodesContent) {
						return yaml.stringify(vibexmodesContent)
					}
					throw new Error("File not found")
				})
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".vibexmodes"),
			...
			it("should import multiple modes at once", async () => {
				...
				expect(result.success).toBe(true)
				expect(vibexmodesContent.customModes).toHaveLength(2)
				expect(vibexmodesContent.customModes[0].slug).toBe("mode1")
				expect(vibexmodesContent.customModes[1].slug).toBe("mode2")
			})
			...
			it("should handle import errors gracefully", async () => {
				...
				// Mock fs.writeFile to work normally for .vibexmodes but we won't get there
				;(fs.writeFile as Mock).mockResolvedValue(undefined)
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				// Verify that no files were written outside the .vibex directory
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".vibexmodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(path.join(mockWorkspacePath, ".vibex"))
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(path.join(".vibex", "rules-test-mode")), {
					recursive: true,
					force: true,
				})
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .vibexmodes file", async () => {
			const vibexmodesContent = { customModes: [{ slug: "other-mode", name: "Other Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
		...
		it("should return false when .vibexmodes doesn't exist and mode is not a custom mode", async () => {
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return false when rules directory doesn't exist", async () => {
			const vibexmodesContent = { customModes: [{ slug: "test-mode", name: "Test Mode" }] }
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockRoomodes
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should return true when rules directory has content files", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.vibex/rules-test-mode" },
			])
		...
		it("should work with global custom modes when .vibexmodes doesn't exist", async () => {
			...
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath // .vibexmodes doesn't exist
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a custom mode in .vibexmodes", async () => {
			const vibexmodesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .vibexmodes", async () => {
			const vibexmodesContent = {
				customModes: [
					{
						slug: "code",
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.vibex/rules-code" },
			])
		...
		it("should successfully export global mode with rules from global .vibex directory", async () => {
			// Mock a global mode
			const globalMode = {
		...
		it("should normalize paths to use forward slashes in exported YAML", async () => {
			...
			rulesFiles.forEach((file: any) => {
				...
				// Files should be at the vibext level now
