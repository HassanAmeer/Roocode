...
describe("CustomModesManager - YAML Edge Cases", () => {
	...
	const mockVibexmodes = `${path.sep}mock${path.sep}workspace${path.sep}.vibexmodes`
	...
	beforeEach(() => {
		...
		;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
			return path === mockSettingsPath || path === mockVibexmodes
		})
  ...
  - slug: "test-mode"
    ...
    groups: ["read"]`
			...
			mockFsReadFile({
				[mockVibexmodes]: yamlWithNonBreakingSpaces,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
  ...
  - slug: "test-mode"
    ...
    groups: ["read"]`
			...
			mockFsReadFile({
				[mockVibexmodes]: yamlWithZeroWidth,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should normalize various quote characters", async () => {
			...
			mockFsReadFile({
				[mockVibexmodes]: yamlWithFancyQuotes,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("Complex fileRegex handling", () => {
		it("should handle complex fileRegex syntax gracefully", async () => {
			...
			mockFsReadFile({
				[mockVibexmodes]: yamlWithComplexFileRegex,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("UTF-8 encoding", () => {
		...
		it("should handle various international characters", async () => {
			...
			mockFsReadFile({
				[mockVibexmodes]: yamlWithInternational,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
	...
	describe("Additional edge cases", () => {
		it("should handle mixed line endings (CRLF vs LF)", async () => {
			...
			mockFsReadFile({
				[mockVibexmodes]: yamlWithMixedLineEndings,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should handle multiple BOMs in sequence", async () => {
			...
			mockFsReadFile({
				[mockVibexmodes]: yamlWithMultipleBOMs,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
		...
		it("should handle deeply nested structures with edge case characters", async () => {
			...
			mockFsReadFile({
				[mockVibexmodes]: yamlWithComplexNesting,
				[mockSettingsPath]: yaml.stringify({ customModes: [] }),
			})
