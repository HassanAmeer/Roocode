...
type VibexSettingsKey = keyof VibexSettings
...
export class ContextProxy {
	 ...
	 */
	private sanitizeProviderValues(values: VibexSettings): VibexSettings {
		...
		for (const key of legacyKeys) {
			if (key in sanitizedValues) {
				...
				sanitizedValues = copy as VibexSettings
			}
		}
		...
		if (values.apiProvider !== undefined && !isProviderName(values.apiProvider)) {
			...
			return restValues as VibexSettings
		}
	...
	public async setValue<K extends VibexSettingsKey>(key: K, value: VibexSettings[K]) {
	...
	public getValue<K extends VibexSettingsKey>(key: K): VibexSettings[K] {
		return isSecretStateKey(key)
			? (this.getSecret(key as SecretStateKey) as VibexSettings[K])
			: (this.getGlobalState(key as GlobalStateKey) as VibexSettings[K])
	}

	public getValues(): VibexSettings {
	...
	public async setValues(values: VibexSettings) {
		const entries = Object.entries(values) as [VibexSettingsKey, unknown][]
