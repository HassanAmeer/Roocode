...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	constructor({
		...
		this.vibexIgnoreController.initialize().catch((error) => {
			console.error("Failed to initialize VibexIgnoreController:", error)
		})
		...
		this.messageQueueStateChangedHandler = () => {
			this.emit(VibexEventName.TaskUserMessage, this.taskId)
		...
		this.debouncedEmitTokenUsage = debounce(
			(tokenUsage: TokenUsage, toolUsage: ToolUsage) => {
				...
				if (tokenChanged || toolChanged) {
					this.emit(VibexEventName.TaskTokenUsageUpdated, this.taskId, tokenUsage, toolUsage)
	...
	private setupProviderProfileChangeListener(provider: ClineProvider): void {
		...
		provider.on(VibexEventName.ProviderProfileChanged, this.providerProfileChangeListener)
	}
	...
	private async addToClineMessages(message: ClineMessage) {
		...
		this.emit(VibexEventName.Message, { action: "created", message })
	...
	private async updateClineMessage(message: ClineMessage) {
		...
		this.emit(VibexEventName.Message, { action: "updated", message })
	...
	private async saveClineMessages() {
		try {
			...
			console.error("Failed to save Vibex messages:", error)
		}
	}
	...
	async ask(
		...
		if (this.abort) {
			throw new Error(`[Vibex#ask] task ${this.taskId}.${this.instanceId} aborted`)
		}
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(VibexEventName.TaskInteractive, this.taskId)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(VibexEventName.TaskResumable, this.taskId)
						}
					}, statusMutationTimeout),
				)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(VibexEventName.TaskIdle, this.taskId)
						}
					}, statusMutationTimeout),
				)
			}
		...
		if (this.idleAsk || this.resumableAsk || this.interactiveAsk) {
			...
			this.emit(VibexEventName.TaskActive, this.taskId)
		}

		this.emit(VibexEventName.TaskAskResponded)
		return result
	}
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[Vibex#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	public async abortTask(isAbandoned = false) {
		...
		this.emit(VibexEventName.TaskAborted)
	...
	public dispose(): void {
		...
		try {
			if (this.providerProfileChangeListener) {
				const provider = this.providerRef.deref()
				if (provider) {
					provider.off(VibexEventName.ProviderProfileChanged, this.providerProfileChangeListener)
				}
		...
		try {
			if (this.vibexIgnoreController) {
				this.vibexIgnoreController.dispose()
				this.vibexIgnoreController = undefined
			}
		} catch (error) {
			console.error("Error disposing VibexIgnoreController:", error)
	...
	private async initiateTaskLoop(userContent: Anthropic.Messages.ContentBlockParam[]): Promise<void> {
		...
		this.emit(VibexEventName.TaskStarted)
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			if (this.abort) {
				throw new Error(`[Vibex#recursivelyMakeRooRequests] task ${this.taskId}.${this.instanceId} aborted`)
			}
			...
			const { content: parsedUserContent, mode: slashCommandMode } = await processUserContentMentions({
				...
				vibexIgnoreController: this.vibexIgnoreController,
				showRooIgnoredFiles,
	...
	public recordToolError(toolName: ToolName, error?: string) {
		...
		if (error) {
			this.emit(VibexEventName.TaskToolFailed, this.taskId, toolName, error)
		}
	}
