...
suite.skip("Vibex use_mcp_tool Tool", function () {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		api.on(VibexEventName.Message, messageHandler)
		...
		api.on(VibexEventName.TaskStarted, taskStartedHandler)
		...
		api.on(VibexEventName.TaskCompleted, taskCompletedHandler)
		await sleep(2000) // Wait for Vibex to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this Vibex won't load the MCP server
		...
		try {
			...
			api.off(VibexEventName.TaskStarted, taskStartedHandler)
			api.off(VibexEventName.TaskCompleted, taskCompletedHandler)
		}
	})
