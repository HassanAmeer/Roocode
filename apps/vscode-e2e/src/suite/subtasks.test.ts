...
suite.skip("Vibex Subtasks", () => {
	test("Should handle subtask cancellation and resumption correctly", async () => {
		const api = globalThis.api

		const messages: Record<string, ClineMessage[]> = {}

		api.on(VibexEventName.Message, ({ taskId, message }: { taskId: string; message: ClineMessage }) => {
		...
		api.on(VibexEventName.TaskSpawned, (_: string, childTaskId: string) => (spawnedTaskId = childTaskId))
