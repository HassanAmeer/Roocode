...
import { type TokenUsage, type ToolUsage, VibexEventName, taskEventSchema } from "@vibex-code/types"
import type { Run, Task, TaskMetrics } from "@vibex-code/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case VibexEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case VibexEventName.TaskTokenUsageUpdated: {
			...
			case VibexEventName.EvalPass:
			case VibexEventName.EvalFail:
